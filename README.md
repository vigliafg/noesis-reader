...
- [Editor Environment](#editor-environment)
...
Noesis Reader includes a companion application: **Noesis Editor** (`noesis-editor.html`, ~3100 lines). It is a full WYSIWYG word processor in the browser, designed to transform extracted chapters and reading notes into polished documents. The Editor communicates with the Reader via shared IndexedDB (`noesisDB`) and `postMessage`, forming a seamless read→edit→export workflow.

### Editor CDN Dependencies

| Library | Version | Purpose |
|---------|---------|---------|
| jQuery | 3.7.1 | DOM manipulation, Summernote dependency |
| Summernote | 0.8.20 | WYSIWYG editor core |
| Turndown | 7.2.2 | HTML → Markdown conversion |
| JSZip | 3.10.1 | MD+ZIP export packaging |
| html-docx-js | 0.3.1 | HTML → DOCX conversion |
| Bootstrap Icons | 1.11.3 | Toolbar and UI icons |

### DOM Structure

```
body
├── #noesis-toolbar (sticky, gradient header)
│   ├── .toolbar-group-left (writing tools)
│   ├── .toolbar-group-center (document tools)
│   ├── .toolbar-group-right (export, tools, help)
│   └── #hamburgerBtn (mobile only)
├── #editor-wrapper (full-height container)
│   └── #summernote (WYSIWYG editor instance)
├── #inspect-panel (draggable, resizable collection inspector)
├── #chunk-fullscreen (expanded chunk preview)
├── #import-overlay (version history browser)
├── #help-overlay (keyboard shortcuts reference)
└── #hamburgerDrawer (mobile menu)
```

### Toolbar & Writing Tools

The toolbar provides 19 formatting tools organized in three groups:

**Left group — Text formatting**:
- Paragraph style (Normal, H1–H6)
- Bold, Italic, Underline, Strikethrough
- Superscript, Subscript
- Font family selector (8 fonts: Arial, Georgia, Times New Roman, Courier New, Verdana, Impact, Comic Sans MS, Trebuchet MS)

**Center group — Document tools**:
...
**Right group — Actions**:
- Export dropdown (7 formats)
- Tools dropdown (Excalidraw)
- Help button

Toolbar event handlers: `src/editor/js/05-init-editor.js` initializes Summernote with custom toolbar configuration; `src/editor/js/07-dropdowns.js` manages dropdown menus.

### Auto-Save & Draft Recovery

**Auto-save** (src/editor/js/01-document.js):
- Saves the editor content to `localStorage` under key `noesis-editor-draft` every 2 seconds
- Only writes when content has actually changed (dirty-checking against last saved state)
- Stores: HTML content, document title, last saved timestamp

**Draft Recovery**:
- On page load, checks `localStorage` for an existing draft
- If found, shows a recovery dialog: "Do you want to recover the draft?"
- Options: "Recover" (restore content), "Discard" (clear draft, start fresh)
- Draft is cleared after successful export

### Export System

`src/editor/js/06-export.js` handles all export operations. The Editor exports in **7 formats**, each generating two variants — **Clean** (publication-ready, no background colors) and **Annotated** (with editor styling preserved):

| Format | File Extension | Method |
|--------|---------------|--------|
| TXT | `.txt` | Strips HTML tags, preserves whitespace |
| Markdown | `.md` | Turndown conversion with custom rules (images, tables, code blocks) |
| MD+ZIP | `.zip` | Markdown + embedded images packaged via JSZip |
| JSON | `.json` | Structured export with `{title, content, timestamp, format}` |
| PDF | `.pdf` | Browser print dialog triggered on a hidden print-optimized iframe |
| DOCX | `.docx` | html-docx-js conversion preserving formatting, tables, images |
| HTML | `.html` | Full standalone HTML document with embedded styles |

**Export flow**:
1. User clicks Export → selects format → enters optional label
2. Clean variant is generated by cloning the editor DOM and removing background colors/styles
3. Annotated variant preserves all visual formatting
4. Both files auto-download (1s delay between them)
5. A snapshot is saved to version history automatically

### Collection Panel (Shared with Reader)

The Editor shares the same `noesisDB` IndexedDB database with the Reader. This allows:
- Chunks (text, images, tables) collected during reading to appear in the Editor
- Chunks collected in the Editor to be available in the Reader

**Collection operations** (`src/editor/js/02-collection.js`, `src/editor/js/03-collection-io.js`):
- **Add to collection**: Select text/image/table → press `+` → saved with book/chapter labels
- **Quick-add images**: Double-click or long-press an image in the editor to add it directly
- **Delete from collection**: Each chunk has a delete button
- **Filter**: Filter by type (text, image, table) or by source book/chapter
- Storage engine: `src/editor/js/00-collection-db.js` provides the IndexedDB abstraction layer

### Inspect Panel

A draggable, resizable floating panel (`src/editor/js/04-inspect.js`) for browsing the collection and inserting items into the document:

- **Open/Close**: Toggle via toolbar button or keyboard shortcut
- **Drag**: Click and drag the header bar to reposition
- **Resize**: Drag the bottom-right corner to resize
- **Content**: Lists all collection items with type icon, source label, and preview
- **Insert**: Each item has an "Insert" button that pastes it at the current cursor position in the editor
- **Filter**: Dropdown to filter by content type
- **Empty state**: "No items in collection" with hint to collect from Reader

### Import & Version History

`src/editor/js/08-import.js` and `src/editor/js/10-snapshots.js`:

**Opening the import overlay**:
- Displays all previously exported versions grouped by: Annotated and Clean
- Each entry shows: title, timestamp, format, optional label
- Original (first export) is highlighted
- Click to load any version into the editor

**Version history**:
- Every export creates an automatic snapshot in `localStorage`
- Snapshots are stored under key `noesis-editor-snapshots` as an array
- Ordered by date, newest first
- Each snapshot contains: full HTML content, title, timestamp, format, label, type (clean/annotated)

### Reader → Editor Bridge

`src/editor/js/09-bridge.js` implements the "Extract + Edit" workflow:

1. User reads a chapter in the Reader and clicks "Extract + Edit"
2. The chapter content, title, and metadata are serialized to JSON
3. A new browser tab opens `noesis-editor.html` with the data passed via `postMessage` or URL hash
4. The Editor receives the payload, loads the content into Summernote, and sets the document title
5. The collection database is synced so all previously collected chunks are available

This creates a seamless flow: **Read → Extract → Edit → Export** without manual copy-paste.

### Excalidraw Integration

The Editor's Tools dropdown includes **Excalidraw** — a custom fork built specifically for Noesis (`noesis-excalidraw`). It uses only the core drawing engine, stripped of cloud, accounts, and collaboration features:

- **Open**: One click from the Editor toolbar → opens in a new tab
- **Draw**: Full drawing toolkit (shapes, arrows, text, freehand, colors)
- **Save**: Drawings saved locally to the browser — no cloud, no account
- **Export**: SVG and PNG formats
- **Import into Editor**: Export as PNG, then insert into the document via the image tool
...
┌─────────────────────────────────────────────────┐
│                  IndexedDB                       │
│  noesisDB.extractedChapters (shared with Reader) │
│  ├── chapter snapshots                           │
│  └── collection items (text, images, tables)     │
└─────────────────────────────────────────────────┘
                        ↕
┌──────────────────┐    ┌────────────────────────┐
│    Reader View    │───▶│    Editor View          │
│  Extract + Edit   │    │  Summernote WYSIWYG     │
│  Collection +     │    │  Export (7 formats)     │
│                   │    │  Inspect panel          │
└──────────────────┘    │  Import/version history │
                        │  Excalidraw drawing     │
                        └────────────────────────┘
                  ...
                  ┌───────────────┴───────────────┐
                  │         localStorage           │
                  │  noesis-editor-draft (auto-save)│
                  │  noesis-editor-snapshots (history)│
                  └───────────────────────────────┘
```
